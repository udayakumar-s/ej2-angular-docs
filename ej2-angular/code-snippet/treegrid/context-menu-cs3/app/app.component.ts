...
export class AppComponent implements OnInit {
    ...
    contextMenuClick(args?: MenuEventArgs): void {
        if(args.element.innerHTML == "Edit Record"){
            this.treegrid.startEdit(args.rowInfo.row);
        }
        else if(args.element.innerHTML == "Delete Record"){
            this.treegrid.deleteRecord(args.rowInfo.row);
        }
    },
    contextMenuOpen(args?: BeforeOpenCloseEventArgs): void {
        if (args.rowInfo.rowData.hasChildRecords == true){
            this.treegrid.grid.contextMenuModule.contextMenu.enableItems(['Edit Record'],true);//Enable edit
            this.treegrid.grid.contextMenuModule.contextMenu.enableItems(['Delete Record'],false);//Disable delete
        } else {
            this.treegrid.grid.contextMenuModule.contextMenu.enableItems(['Edit Record'],false);//Disable edit
            this.treegrid.grid.contextMenuModule.contextMenu.enableItems(['Delete Record'],true);//Enable edit
        }
    }
}
